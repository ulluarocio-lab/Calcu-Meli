...
def analizar_competencia_api(busqueda_limpia):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not resultados: return "SIN_RESULTADOS"
            
            mercado_lideres, envios_full, precios = 0, 0, []
            for item in resultados:
                precios.append(item.get("price", 0))
                reputacion = item.get("seller", {}).get("seller_reputation", {}).get("power_seller_status")
                if reputacion in ["platinum", "gold", "silver"]: mercado_lideres += 1
                if item.get("shipping", {}).get("logistic_type") == "fulfillment": envios_full += 1
                    
            return {
                "termino_buscado": busqueda_limpia, "total_analizados": len(resultados),
                "mercado_lideres": mercado_lideres, "envios_full": envios_full,
                "precio_promedio": sum(precios) / len(precios) if precios else 0
            }
    except Exception: return None

def obtener_comision(tipo_pub):
    return 0.15 if tipo_pub == "Clásica (Sin cuotas)" else 0.20 if tipo_pub == "Premium (3 Cuotas)" else 0.25
...
def calcular_precio_sugerido(costo, tipo, cond, envio_gratis, margen_deseado, acos_pct):
    ...
    denominador = 1 - (porcentaje_comision * factor_iva_meli) - porcentaje_impuestos - (acos_pct / 100.0) - (margen_deseado / 100.0)
...
def guardar_producto(nombre, costo, precio, ganancia, margen, roi, unidades, inversion, facturacion):
    st.session_state.portafolio.append({
        "Producto": nombre if nombre else "Sin Nombre", "Costo Unit.": costo, "Precio Venta": precio,
        "Ganancia Unit.": round(ganancia, 2), "Margen (%)": round(margen, 1), "ROI (%)": round(roi, 1),
        "Unidades/Mes": unidades, "Inversión Req.": inversion, "Facturación Est.": facturacion
    })
...
def evaluar_catalogo_mascotas(nombre_producto):
    ...
    palabras_estrella = ['kit', 'combo', 'set', 'interactivo', 'lamido', 'comedero lento', 'kong', 'dispensador', 'cepillo', 'saca', 'portátil', 'entrenamiento', 'rascador', 'dental']
...
with st.sidebar:
    ...
    if producto_nombre:
        if "http" in producto_input: st.success(f"🔗 Link convertido a: **{producto_nombre}**")
...
if costo_input is not None:
    ...
    if precio_input is None or precio_input == 0:
        if precio_sugerido > 0: precio, modo_color, modo_msj = precio_sugerido, "success", f"🤖 **Modo Automático:** Analizando precio sugerido de **${precio:,.0f}**"
        else: st.error(f"❌ Imposible alcanzar {margen_obj}% de margen."); st.stop()
    else:
        precio, modo_color, modo_msj = precio_input, "info", f"⚙️ **Modo Manual:** Analizando precio de **${precio:,.0f}**"
    ...
    with tab1:
        ...
        if datos_api == "SIN_RESULTADOS":
            ...
            if es_estrella:
                ...
                st.markdown(f"""
                **Fundamentos:**
                * **Rentabilidad:** Margen óptimo ({mar:.1f}%) y alto ROI ({mkp:.1f}%).
                ...
                * **Acción Sugerida:** **Descartar compra o reestructurar.** Requiere armar un Kit.
                """)
            else:
                ...
                * **Acción Sugerida:** **Prueba piloto.** Avanzar solo con un stock mínimo.
                """)
        else:
...
with tab5:
    ...
    if archivo_catalogo is not None:
        try:
            if archivo_catalogo.name.endswith('.csv'): df_cat = pd.read_csv(archivo_catalogo)
            else: df_cat = pd.read_excel(archivo_catalogo)
            
            st.success("✅ Catálogo cargado correctamente.")
            
            columnas = df_cat.columns.tolist()
            col_nombre = st.selectbox("¿Qué columna tiene el Nombre del Producto?", columnas)
            ...
            if st.button("🔍 Iniciar Escáner de Viabilidad", type="primary"):
                with st.spinner("Analizando cientos de productos..."):
                    ...
                    df_neutro = df_cat[df_cat['Recomendación ML'].str.contains("Neutro")]
            ...
            st.error(f"Error al leer el archivo: Comprueba que no tenga formato corrupto.")
